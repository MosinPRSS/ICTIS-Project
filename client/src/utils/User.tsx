...
import { Edit2, Trash2, Heart, Bot, ChevronLeft, ChevronRight, Frown } from 'lucide-react';
...
const UserProfile: React.FC = () => {
  ...
  const getFilteredBots = () => {
    ...
    switch (activeFilter) {
      case 'public': filtered = userBots.filter(bot => bot.isPublic); break;
      case 'private': filtered = userBots.filter(bot => !bot.isPublic); break;
      case 'favorites': filtered = userBots.filter(bot => bot.likes > 30); break;
      default: filtered = userBots;
    }
  ...
  const totalFilteredBots = userBots.filter(bot => {
    switch (activeFilter) {
      case 'public': return bot.isPublic;
      case 'private': return !bot.isPublic;
      case 'favorites': return bot.likes > 30;
      default: return true;
    }
  }).length;
  ...
  return (
    <div className="w-full p-4 pt-2">
      <div className="max-w-6xl mx-auto mb-4">
        <h1 className="text-3xl font-bold text-white text-center">Профиль пользователя</h1>
      </div>

      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-4 h-[calc(100vh-120px)]">
        <div className="lg:col-span-1 h-full">
          <div className={`${theme.options.bgColor3} backdrop-blur-sm rounded-2xl p-6 ${theme.options.bgBorderColor} h-full flex flex-col`}>
            <div className="flex flex-col items-center gap-4 mb-6">
              ...
              <div className="text-center">
                ...
                <p className="text-purple-400 text-xs mt-1">{currentDate}</p>
              </div>
            </div>

            <div className="mb-6 flex-1">
              <h3 className="text-purple-300 text-sm font-medium mb-2">Описание</h3>
              {isEditing ? (
                <textarea
                  value={editForm.description}
                  onChange={handleDescriptionChange}
                  placeholder="Описание отсутствует"
                  className={`w-full ${theme.options.bgColor} rounded-lg p-4 text-white min-h-[120px] resize-y border-2 border-purple-400/50 backdrop-blur-sm shadow-lg shadow-purple-500/20 focus:border-purple-400 focus:shadow-purple-500/40 transition-all duration-200`}
                  style={{
                    minHeight: '120px',
                    maxHeight: '300px'
                  }}
                ...
                <div className={`${theme.options.mgColor} rounded-lg p-4 text-white min-h-[120px] max-h-[300px] overflow-y-auto ${theme.options.bgBorderColor} backdrop-blur-sm`}>
          ...
          <div className={`${theme.options.bgColor3} backdrop-blur-sm rounded-2xl p-6 ${theme.options.bgBorderColor} h-full flex flex-col`}>
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs gap-2 mt-4">
                    <span className={`px-2 py-1 rounded-full ${
                      ...
                      {bot.isPublic ? 'Публичный' : 'Приватный'}
                    </span>
                    <div className="flex items-center gap-1 text-pink-300">
                      <Heart size={12} />
                      <span className="text-xs">{bot.likes}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            {filteredBots.length === 0 && (
              <div className="text-center py-12 flex-1 flex flex-col items-center justify-center">
                <Frown size={48} className="text-purple-400 mb-4" />
                <p className="text-purple-300 text-lg mb-2">У вас пока нет ботов</p>
              </div>
            )}
            ...
            {totalFilteredBots > 6 && (
              <div className="flex justify-center items-center gap-2 mt-auto pt-4">
                ...
                <div className="flex items-center gap-1">
                  {getPageNumbers().map((page, index) => (
                    <React.Fragment key={index}>
                      {page === '...' ? (
                        <span className="px-2 py-1 text-purple-400">...</span>
                      ) : (
                        <button
                ...
                <button
                  onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPagesForFilter))}
                  disabled={currentPage === totalPagesForFilter}
                  className="p-2 rounded-lg bg-purple-900/50 text-purple-300 hover:text-white hover:bg-purple-800/50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
